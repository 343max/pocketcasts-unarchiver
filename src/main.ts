...
const main = async () => {
  ...
  const podcastList = await pocketcasts.podcastList()
  const podcastsToUnarchive = config.POCKET_CASTS_PODCAST_TITLE.split(",").map((t) => t.trim())
  for (const podcastTitle of podcastsToUnarchive) {
    const podcast = podcastList.podcasts.filter(({ title }) => title.includes(podcastTitle))[0]
    ...
    for (const episodeUuid of startedEpisodes) {
      await pocketcasts.updateEpisode(episodeUuid, podcast.uuid, {
        position: "0",
        status: 2,
      })
